...
def _search_fulltext(repo_path, query: str, limit: int) -> None:
    async def _run():
        from repowise.core.persistence import FullTextSearch, create_engine, create_session_factory, get_session
        ...
        results = await fts.search(query, limit=limit, repository_id=repo.id if repo else None)
        ...
        async with get_session(sf) as session:
            ...
            result = await session.execute(
                sa_text(
                    ...
                    "FROM wiki_symbols "
                    "WHERE repository_id = :repo_id AND name LIKE :pattern "
                    "LIMIT :limit"
                ),
                {"repo_id": repo.id, "pattern": f"%{query}%", "limit": limit},
            )
