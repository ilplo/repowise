...
async def _persist_result(
    ...
    if fts is not None and result.generated_pages:
        for page in result.generated_pages:
            await fts.index(make_storage_page_id(repo.id, page.page_id), page.title, page.content)
...
def init_command(
    ...
    if commit_limit is not None:
        ...
        save_config(
            ...
            cfg.get("provider", ""),
            cfg.get("model", ""),
            cfg.get("embedder", embedder_name_resolved),
            exclude_patterns=cfg.get("exclude_patterns"),
    ...
    if index_only:
        ...
        next_steps = [
            ...
            ("repowise init --provider xai", "generate full documentation"),
