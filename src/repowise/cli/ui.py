...
from repowise.app_runtime import ensure_app_data_dir, get_global_env_path
...
# ---------------------------------------------------------------------------
# .env persistence  —  save/load API keys in the central app .env
# ---------------------------------------------------------------------------


def load_dotenv(repo_path: Path) -> None:
    """Load the central app ``.env`` into ``os.environ`` (without overwriting)."""
...
def _save_key_to_dotenv(repo_path: Path, env_var: str, value: str) -> None:
    """Append or update a key in the central app ``.env``."""
    ensure_app_data_dir()
    env_file = get_global_env_path()
...
def _prompt_api_key(
    ...
    *,
    ...
    """Prompt for an API key, set env var, and optionally save to the app .env.
    ...
    if repo_path is not None:
        save = click.confirm(
            "  Save key to the repowise app .env for future runs?",
            default=True,
        )
        if save:
            _save_key_to_dotenv(repo_path, env_var, key)
            console.print(f"  [{OK}]✓ Saved to {get_global_env_path()}[/]")
    console.print()
