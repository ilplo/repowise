...
async def get_page_by_query(
    ...
    repo_id: str | None = Query(None, description="Repository ID for logical page IDs"),
    session: AsyncSession = Depends(get_db_session),  # noqa: B008
) -> PageResponse:
...
async def get_page_versions_by_query(
    ...
    versions = await crud.get_page_versions(session, page_id, repository_id=repo_id, limit=limit)
