...
def _validated_repo_local_path(local_path: str) -> str:
    try:
        return str(validate_target_repo_path(local_path))
    except TargetRepoResolutionError as error:
        raise HTTPException(status_code=400, detail=str(error)) from error


def _ensure_repo_source_available(local_path: str) -> None:
    try:
        validate_target_repo_path(local_path)
    except TargetRepoResolutionError as error:
