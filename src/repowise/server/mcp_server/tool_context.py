...
async def _resolve_one_target(
    ...
    page = await get_page(session, page_id, repository_id=repo_id)
    target_type = None
    ...
    if include is None or "docs" in include:
        ...
        if target_type == "file":
            ...
            sym_page = await get_page(session, sym_page_id, repository_id=repo_id)
            if sym_page is not None:
                docs["page_id"] = sym_page.id
