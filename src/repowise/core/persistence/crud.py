...
def _split_logical_page_id(page_id: str) -> tuple[str, str] | None:
    parsed = parse_storage_page_id(page_id)
    logical_page_id = parsed.logical_page_id if parsed is not None else page_id
    if ":" not in logical_page_id:
        return None
    page_type, target_path = logical_page_id.split(":", 1)
    if not page_type:
        return None
...
async def upsert_repository(
    ...
    *,
    ...
    """
    canonical_path = str(canonicalize_target_repo_path(local_path))
    result = await session.execute(select(Repository).where(Repository.local_path == canonical_path))
...
async def upsert_page(
    ...
    *,
    ...
    storage_page_id = make_storage_page_id(repository_id, page_id)
    existing_result = await session.execute(select(Page).where(Page.id == storage_page_id))
...
async def get_page(
    ...
    """Return a page by storage ID, or by logical ID when ``repository_id`` is provided."""
    ...
    parts = _split_logical_page_id(page_id)
    if parts is None:
        return None
    page_type, target_path = parts
...
async def get_page_versions(
    ...
    *,
    ...
    page = await get_page(session, page_id, repository_id=repository_id)
    if page is None:
        return []
    result = await session.execute(
        select(PageVersion)
