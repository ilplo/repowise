...
from .page_ids import get_logical_page_id
...
class Page(Base):
    ...
    The primary key is a repo-scoped storage ID. The logical/generated page ID
    remains ``"{page_type}:{target_path}"`` and can be recovered via the
    ``logical_page_id`` property.
    """
    ...
    @property
    def logical_page_id(self) -> str:
        return get_logical_page_id(self.id)
...
class GraphNode(Base):
    ...
    # Keep the ORM aligned with newer graph_nodes schemas that annotate
    # communities and symbol-derived metadata on file nodes.
