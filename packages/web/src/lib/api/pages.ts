...
export async function getPageById(pageId: string, repoId?: string): Promise<PageResponse> {
  return apiGet<PageResponse>("/api/pages/lookup", { page_id: pageId, repo_id: repoId });
}
...
export async function regeneratePage(pageId: string, repoId?: string): Promise<{ job_id: string }> {
